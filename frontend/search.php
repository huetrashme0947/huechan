...
<head>
	...
	<script>
		...
		function initPage(first = true) {
			...
			<?php endif; ?>
				document.getElementById("search-bar").value = "<?=htmlspecialchars($_GET["q"])?>";
				loadContent(false);
			<?php if ($uid == null): ?>
				}
			<?php endif; ?>
		...
		function loadContent(append = true) {
			...
			http.open("GET", "/ext/filter_posts?<?=($board_id != null ? "by_board=$board_id&" : "")?>by_text=<?=htmlspecialchars($_GET["q"])?>&return_full=1"+(append ? "&offset_time="+lastPostTime : ""));
...
<body class="text-light" style="background-color: #171717">
	<?=retrieveNavbar(($board_id != null ? $board_id : null), getBanType($database, $uid, $user_ip), $user_role !== null)?>
	...
	<div class="container-sm">
		<p><h1>Search (<?=$board_id != null ? '/'.$board_id.'/' : "Global"?>)</h1></p>
